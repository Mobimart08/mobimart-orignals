...
/* ==========================================================================
   ...
   - Connected to centralized products.js — no hardcoded data
...
export const FeaturedProducts = () => {
  // Pull real data from the centralized store
  const featuredHero   = products[0]; // iPhone 15 Pro — top hero
  const gridProducts   = products.slice(1, 4); // next 3 for the bottom grid
    ...
    <section className="w-full pb-8 bg-white">
      <Container>
        ...
        <div className="flex flex-col gap-4 sm:gap-6">
          ...
          <Link to={`/product/${featuredHero.id}`} className="block group">
            <Card
              variant="custom"
            ...
            >
              ...
              <div className="flex-1 flex flex-col items-start text-left z-10 pl-2 sm:pl-4">
                {/* Condition badge */}
                <span className={`text-[8px] sm:text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded mb-2 sm:mb-3 ${
                  ...
                  {featuredHero.condition}
                </span>

                <h3 className="text-base sm:text-2xl lg:text-3xl font-extrabold text-neutral-950 tracking-tight leading-tight mb-1 sm:mb-2">
                  {featuredHero.name}
                </h3>
                ...
                <div className="flex items-center gap-1 mb-2 sm:mb-4">
                  ...
                  <span className="text-[9px] sm:text-xs font-bold text-neutral-700">
                    {featuredHero.rating} · {featuredHero.reviewCount} reviews
                  </span>
                </div>

                {/* Price row */}
                <div className="flex items-baseline gap-2 mb-3 sm:mb-5">
                  <span className="text-sm sm:text-xl font-black text-neutral-950">
                    {featuredHero.price}
                  </span>
                ...
                <Button
                  variant="primary"
                  size="sm"
                  className="cursor-pointer !px-4 !py-1.5 !text-[8px] sm:!text-xs flex items-center gap-1 hover:brightness-90 transition-all"
                >
                  Shop Now
                  <ArrowRight size={10} />
                </Button>
