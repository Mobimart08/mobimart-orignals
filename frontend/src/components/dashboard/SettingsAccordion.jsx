...
import { User, Phone, Mail, Lock, ShieldCheck, LogOut, ChevronDown, Check, Trash2, Eye, EyeOff } from 'lucide-react';
...
export const SettingsAccordion = ({ profile, onUpdateProfile, onLogout }) => {
  ...
  const handlePasswordUpdate = async () => {
    ...
    try {
      await userService.updatePassword({ currentPassword: password, newPassword });
      setPassword('');
      setNewPassword('');
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-[#ECEFF2]/30 focus:bg-white text-[11px] sm:text-xs text-neutral-850 pl-3.5 pr-4 py-2 rounded-xl border border-neutral-200/50 focus:outline-none focus:border-[#C5A880] transition-all font-mono pr-10"
              />
              <button
                type="button"
          ...
          <div className="flex items-center justify-between gap-4 p-2 bg-[#FAF9F6] border border-neutral-100 rounded-xl">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input 
                ...
                onChange={() => {
                  setIs2FA(!is2FA);
                  showToast(!is2FA ? '2-Factor Authentication Enabled' : '2-Factor Authentication Disabled', 'info');
                }}
      ...
      render: () => (
        <div className="flex flex-col gap-2 pt-1 text-left">
          ...
          <button
            ...
            disabled={isDeleting}
            className="w-full py-3 bg-red-600 hover:bg-red-700 text-white text-xs font-black rounded-xl cursor-pointer mt-1 flex items-center justify-center gap-1.5 disabled:opacity-50"
          >
