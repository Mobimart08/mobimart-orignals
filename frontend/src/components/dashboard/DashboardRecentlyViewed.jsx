...
export const DashboardRecentlyViewed = React.memo(() => {
      ...
      <div className="w-full overflow-x-auto no-scrollbar py-1">
        <div className="flex items-stretch gap-3.5">
          {recentlyViewed.map((prod) => {
            const imageSrc = prod.images?.[0]?.url || '/placeholder.png';
            const priceFormatted = `₹${(prod.price || 0).toLocaleString('en-IN')}`;
            const ratingFormatted = prod.averageRating ? prod.averageRating.toFixed(1) : 'New';
